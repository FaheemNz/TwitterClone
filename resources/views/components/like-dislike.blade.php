<div class="card-footer d-flex social-button">
    <form class="mr-3" method="POST" action="/tweets/{{ $tweet->id }}/like">
        @csrf
        <button type="submit">
            <svg viewBox="0 0 20 20">
                <g stroke="none" stroke-width="1" fill="{{ $tweet->islikedBy(auth()->user()) ? 'blue' : 'gray' }}" fill-rule="evenodd">
                    <g id="icon-shape">
    ...
    <form action="/tweets/{{ $tweet->id }}/like" method="POST">
        ...
        <button type="submit">
            <svg viewBox="0 0 20 20">
                <g stroke="none" stroke-width="1" fill="{{ $tweet->isDislikedBy(auth()->user()) ? 'blue' : 'gray' }}" fill-rule="evenodd">
                    <g id="icon-shape">
